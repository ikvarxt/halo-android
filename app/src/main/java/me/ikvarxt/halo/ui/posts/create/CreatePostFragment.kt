...
class CreatePostFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.bottomBar.setOnMenuItemClickListener {
            when (it.itemId) {
                R.id.insertAsset -> {
                    AddAssetFragment().show(parentFragmentManager, AddAssetFragment.TAG)
                }
                else -> {}
            }
            true
        ...
        setFragmentResultListener(INSERT_ASSET_REQUEST_KEY) { key, bundle ->
            val assetSchema = bundle.getString(ASSET_KEY)
